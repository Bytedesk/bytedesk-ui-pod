...
@property(nonatomic, strong) NSString *tId; // 统一代表：thread.tid/contact.uid/group.gid
@property(nonatomic, strong) NSString *visitorUid;
@property(nonatomic, strong) NSString *mChatType;
...
- (void)initWithUid:(NSString *)uId wId:(NSString *)wId withTitle:(NSString *)title withPush:(BOOL)isPush {
    ...
    self.uId = uId;
    self.wId = wId;
    self.visitorUid = [BDSettings getUid];
    self.mChatType = BD_THREAD_TYPE_THREAD;
    self.rateScore = 5;
    self.rateNote = @"";
                                ...
                                NSLog(@"%s, %@", __PRETTY_FUNCTION__, dict);
                                  ...
                                  */
                                  ...
                                  if ([status_code isEqualToNumber:[NSNumber numberWithInt:200]] ||
                                      ...
                                      [status_code isEqualToNumber:[NSNumber numberWithInt:206]]) {
                                      // 创建新会话
                                      // 继续进行中会话
                                      // 为兼容老版本，服务器增加206，可去掉
                                      ...
                                      self.tId = self.mThreadModel.tid;
                                      self.threadTopic = [NSString stringWithFormat:@"thread/%@", self.tId];
...
- (void)initWithThreadModel:(BDThreadModel *)threadModel withPush:(BOOL)isPush {
    ...
    self.tId = self.mThreadModel.tid;
    self.visitorUid = threadModel.visitor_uid;
    self.mChatType = BD_THREAD_TYPE_THREAD;
    self.m_imagePickerController = [[UIImagePickerController alloc] init];
    self.m_imagePickerController.delegate = self;
    ...
    // TODO: 根据会话类型区分：thread/contact/group会话
...
- (void) initWithContactModel:(BDContactModel *)contactModel withPush:(BOOL)isPush {
    ...
    self.tId = self.mContactModel.uid;
    self.mChatType = BD_THREAD_TYPE_CONTACT;
    self.m_imagePickerController = [[UIImagePickerController alloc] init];
    self.m_imagePickerController.delegate = self;
...
- (void) initWithGroupModel:(BDGroupModel *)groupModel withPush:(BOOL)isPush {
    ...
    self.tId = self.mGroupModel.gid;
    self.mChatType = BD_THREAD_TYPE_GROUP;
    self.m_imagePickerController = [[UIImagePickerController alloc] init];
    self.m_imagePickerController.delegate = self;
...
- (void)reloadTableData {
    ...
    //
    if ([self.mChatType isEqualToString:BD_THREAD_TYPE_THREAD]) {
        
        self.mMessageArray = [BDCoreApis getMessagesWithThread:self.visitorUid];
    } else if ([self.mChatType isEqualToString:BD_THREAD_TYPE_CONTACT]) {
        
        self.mMessageArray = [BDCoreApis getMessagesWithContact:self.mContactModel.uid];
    } else if ([self.mChatType isEqualToString:BD_THREAD_TYPE_GROUP]) {
        
        self.mMessageArray = [BDCoreApis getMessagesWithGroup:self.mGroupModel.gid];
    }
...
-(void)sendMessage:(NSString *)content {
    ...
    //
    [[BDMQTTApis sharedInstance] sendTextMessage:content toTid:self.tId sessionType:self.mChatType];
}
...
- (void)sendImage:(NSData *)imageData {
    ...
    [BDCoreApis uploadImageData:imageData withImageName:imageName resultSuccess:^(NSDictionary *dict) {
        ...
        [[BDMQTTApis sharedInstance] sendImageMessage:imageUrl toTid:self.tId sessionType:self.mChatType];
    } resultFailed:^(NSError *error) {
        NSLog(@"%s %@", __PRETTY_FUNCTION__, error);
    }];
}
